...
static const char INIT_SCRIPT[] =
    ...
    "_MAGIC = b'UVPKLAUN'\n"
    "_TAIL = _struct.Struct('<I8s')\n"
    "_TAIL_LEN = _TAIL.size\n"
    "with open(_SELF, 'rb') as _fp:\n"
    "    _fp.seek(-_TAIL_LEN, 2)\n"
    "    _json_len, _magic = _TAIL.unpack(_fp.read(_TAIL_LEN))\n"
    "    if _magic != _MAGIC:\n"
    "        raise RuntimeError('invalid launcher payload trailer')\n"
    "    _fp.seek(-(_TAIL_LEN + _json_len), 2)\n"
    "    _meta = _json.loads(_fp.read(_json_len).decode('utf-8'))\n"
    "    _archive_size = int(_meta['archive_size'])\n"
    "    _fp.seek(-(_TAIL_LEN + _json_len + _archive_size), 2)\n"
    "    _archive_bytes = _fp.read(_archive_size)\n"
    "    if len(_archive_bytes) != _archive_size:\n"
    "        raise RuntimeError('truncated embedded zip')\n"
    "_ENTRY_MODULE = _meta['module']\n"
    "_ENTRY_FUNC = _meta.get('func', 'main')\n"
    "class _MemZip:\n"
