...
from uvpacker.domain.rules import require_exact_minor_from_requires
from uvpacker.domain.sources import PackDownloadConfig
from uvpacker.infra import runtime_cache, runtime_downloader
...
class TestCollectCandidatesForMinor:
    def test_single_candidate(self) -> None:
        html = "3.12.1/"
        result = runtime_downloader._collect_candidates_for_minor(html, "3.12")
    ...
    def test_two_digit_minor(self) -> None:
        html = "3.10.12/"
        result = runtime_downloader._collect_candidates_for_minor(html, "3.10")
...
class TestIsValidCachedZip:
    def test_valid_zip(self, tmp_path: Path) -> None:
        ...
        assert runtime_cache._is_valid_cached_zip(p)

    def test_non_existent_file(self, tmp_path: Path) -> None:
        assert not runtime_cache._is_valid_cached_zip(tmp_path / "missing.zip")

    def test_empty_file(self, tmp_path: Path) -> None:
        p = tmp_path / "empty.zip"
        p.write_bytes(b"")
        assert not runtime_cache._is_valid_cached_zip(p)
    ...
    def test_directory(self, tmp_path: Path) -> None:
        d = tmp_path / "dir"
        d.mkdir()
        assert not runtime_cache._is_valid_cached_zip(d)
...
class TestResolveLatestEmbedForMinor:
    def test_resolves_highest_patch_with_available_zip(
        ...
        result = runtime_downloader.resolve_latest_embed_for_minor("3.12")
        assert result == "3.12.3"

    def test_no_candidates_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(runtime_downloader, "_fetch_text", lambda url, action: "")
        with pytest.raises(RuntimeResolveError, match="No patch releases found"):
            runtime_downloader.resolve_latest_embed_for_minor("3.99")
...
class TestDownloadAndExtractEmbeddedRuntime:
    ...
    def test_stale_lock_is_recovered(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(runtime_cache, "get_embed_cache_dir", lambda: cache_dir)
        cache_dir.mkdir(parents=True)
        lock_path = cache_dir / "python-3.12.1-embed-amd64.zip.lock"
        lock_path.write_text("stale")
        stale_time = time.time() - (runtime_cache._LOCK_STALE_SECONDS + 60)
